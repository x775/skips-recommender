...
def eval_sknn(trained_sknn, start_idx, end_idx, num_of_tracks, nn=10, topk=10, batch_size=1000):
    ...
    for i in range(start_idx, end_idx, batch_size):
        batches.append(test_sessions_list[i:i + batch_size])
...
if __name__ == "__main__":
    ...
    if "allskips" in sys.argv:
        ...
        # Load all data as dataframes.
        ABSOLUTE_PATH = str(Path(__file__).resolve().parents[1].absolute()) + "{0}data{0}".format(os.sep)
    ...
    _, train_sessions, _, test_sessions = data_handling.get_split(users, sessions, TRAIN_TEST_SPLIT, VALIDATION_SPLIT)
    ...
    if sys.argv[1] == "train":
        ...
        sparse.save_npz("trained_sknn_tf.npz", trained_sknn)
    if sys.argv[1] == "eval":
        # Load the saved training sparse matrix; in csr format.
        trained_sknn = sparse.load_npz("trained_sknn_tf.npz")
    ...
    for neighbour in neighbours:
        for k in topk:
            ...
            with open("sknn_results.txt", "a") as out:
