...
def get_split(user_data, session_data, train_split, validation_split=None):
    ...
    train_skips = train_session_data.skips.values.tolist()
    train_pairs = list(zip(train_tracks, train_tags, train_skips))

    test_tracks = test_session_data.track_idxs.values.tolist()
    test_tags = test_session_data.tags_idxs.values.tolist()
    test_skips = test_session_data.skips.values.tolist()
    test_pairs = list(zip(test_tracks, test_tags, test_skips))
    ...
    if validation_split:
        ...
        val_skips = val_session_data.skips.values.tolist()
        val_pairs = list(zip(val_tracks, val_tags, val_skips))
