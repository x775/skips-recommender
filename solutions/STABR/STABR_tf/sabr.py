...
class STABR_forward(tf.keras.layers.Layer):
    ...
    def call(self, songs_context, training=True):
        vec_rep = self.v_layer(songs_context)
        if training:
...
class STABR(tf.keras.Model):
    def __init__(self, song_input_size, song_embedding_size, hidden_size=64,
    ...
    def call(self, input_songs, training=True):
        songs_context = self.song_encoder(input_songs)
        res_logits = self.stabr_forward(songs_context, training)
