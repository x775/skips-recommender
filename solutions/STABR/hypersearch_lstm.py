...
from STABR_skip import data_handling_tf, stabr_skip_lstm, stabr_skip_lstm_tags, stabr_skip_mix
...
with open(ABSOLUTE_PATH + "{0}data{0}skips{0}sessions_10sessions_2plays_binaryskip_tf.json".format(os.sep), "r") as source:
    sessions = pd.read_json(source, orient="index")

with open(ABSOLUTE_PATH + "{0}data{0}skips{0}tracks_10sessions_2plays_binaryskip_tf.json".format(os.sep), "r") as source:
    songs = pd.read_json(source, orient="index")
    songs.index = songs.index + 1

with open(ABSOLUTE_PATH + "{0}data{0}skips{0}users_10sessions_2plays_binaryskip_tf.json".format(os.sep), "r") as source:
    users = pd.read_json(source, orient="index")

print("Created dataframes...")

# Grab total number of tags.
with open(ABSOLUTE_PATH + "{0}data{0}skips{0}tags_10sessions_2plays_binaryskip_tf.json".format(os.sep)) as source:
...
val_sessions["skips"] = val_sessions["skips"].apply(lambda idxs: idxs[:-1])
...
train_dataset = train_dataset.padded_batch(32, padded_shapes=(([None], [None, None], [None]), []))
train_dataset = train_dataset.prefetch(tf.data.experimental.AUTOTUNE)

val_dataset = tf.data.Dataset.from_generator(val_generator, 
              output_types=((tf.int32, tf.int32, tf.int32), tf.int32),
              output_shapes=(((None,), (None, None,), (None,)), ()))
val_dataset = val_dataset.padded_batch(100, padded_shapes=(([None], [None, None], [None]), []))
...
def build_model(hp):
    model = stabr_skip_lstm.STABR(
        ...
        SKIP_ACTIONS + 1,
        hp.Choice("skip_embedding_size", values=[5, 10, 15], default=5),
...
best_model.save_weights("hyper_search_lstm_skips/model", format="tf")
