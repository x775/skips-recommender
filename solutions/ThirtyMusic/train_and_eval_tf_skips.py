...
from solutions.ThirtyMusic import stabr_skip_lstm as stabr_skip_tf
from solutions.ThirtyMusic import data_handling_tf_skip
...
def run_train_test():
    ...
    for index in range(3):
        ...
        train_sessions = data_handling_tf_skip.get_subsessions(train_data)
        val_sessions = data_handling_tf_skip.get_subsessions(val_data)
        test_sessions = data_handling_tf_skip.get_subsessions(pd.read_json(TESTING_SET, orient="index"))
        ...
        train_dataset = train_dataset.padded_batch(BATCH_SIZE, padded_shapes=(([None], [None, None], [None]), []))
        ...
        val_dataset = tf.data.Dataset.from_generator(val_generator, 
                      output_types=((tf.int32, tf.int32, tf.int32), tf.int32),
                      output_shapes=(((None,), (None, None,), (None,)), ()))
        val_dataset = val_dataset.padded_batch(VAL_BATCH_SIZE, padded_shapes=(([None], [None, None], [None]), []))

    
        stabr = stabr_skip_tf.STABR(TOTAL_SONGS + 1, 
                                    SONG_EMBEDDING_SIZE,
                                    TOTAL_TAGS + 1, 
                                    TAG_EMBEDDING_SIZE,
                                    SKIP_ACTIONS + 1, 
        ...
        checkpoint_cb = tf.keras.callbacks.ModelCheckpoint(
            "stabr_skip_30music_models/stabr_skips_30music_set" + str(index) + "_{epoch}.ckpt",
        ...
        if "train" in sys.argv:
            ...
            eval_model_path = "stabr_skip_30music_models{}stabr_skips_30music_set{}_{}.ckpt".format(os.sep, index, sys.argv[2])
            ...
            EPOCH = sys.argv[2]
            eval_model_path = "stabr_skip_30music_models{}stabr_skips_30music_set{}_{}.ckpt".format(os.sep, index, EPOCH)
            stabr.compile(optimizer=optimizer, 
                          loss=loss_fn, 
                          metrics=[tf.keras.metrics.SparseTopKCategoricalAccuracy(k=10)])
            init_data = iter(train_dataset).next()
