...
from solutions.ThirtyMusic import sabr
...
def run_train_test():
    ...
    for index in range(3):
        ...
        val_dataset = tf.data.Dataset.from_generator(val_generator, 
                    output_types=(tf.int32, tf.int32), output_shapes=((None,), ()))
        val_dataset = val_dataset.padded_batch(TEST_BATCH_SIZE, padded_shapes=([None], []))
        ...
        stabr = sabr.STABR(TOTAL_SONGS + 1, 
                           SONG_EMBEDDING_SIZE, 
                           HIDDEN_SIZE, 
                           DROPOUT, 
                           V_LAYER_SIZE)
        ...
        checkpoint_cb = tf.keras.callbacks.ModelCheckpoint(
            "sabr_30music_models/sabr_{epoch}.ckpt",
        ...
        if "train" in sys.argv:
            ...
            eval_model_path = "stabr_30music_models{}stabr_30music_set{}_{}.ckpt".format(os.sep, index, sys.argv[2])
            ...
            EPOCH = sys.argv[2]
            eval_model_path = "stabr_30music_models{}stabr_30music_set{}_{}.ckpt".format(os.sep, index, EPOCH)
            init_data = iter(train_dataset).next()
            inp, target = init_data
            stabr.train_on_batch(inp, target)
            stabr.load_weights(eval_model_path)
